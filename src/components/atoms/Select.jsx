...
const CustomOption = ({ children, isSelected, isFocused, ...props }) => {
  return (
    <components.Option {...props}>
      <div
        className={`${isFocused && "bg-gray-100 active:bg-gray-200 dark:bg-gray-600 dark:active:bg-gray-700"} text-gray-500 dark:text-gray-400 hover:cursor-pointer px-3 py-2 rounded flex justify-between items-center`}
      >
        <div className="flex flex-col">
          <span>{children}</span>
        </div>
        {isSelected && <Icon path={mdiCheck} size={0.8} className="text-green-500" />}
      </div>
    </components.Option>
  );
};

export default function Select({async = false, color = "gray", helperText, ...props}) {
    ...
    <div>
      <SelectComponent
        ...
        classNames={{
          ...
          placeholder: () => `${color === "failure" ? "text-red-500" : "text-gray-500"} pl-1 py-0.5`,
          input: () => "pl-1 py-0.5",
          ...
          menu: () => "p-1 mt-2 border border-gray-200 bg-white dark:border-gray-600 dark:bg-gray-800 rounded-lg",
          groupHeading: () => "ml-3 mt-2 mb-1 text-gray-500 text-sm",
          noOptionsMessage: () => "text-gray-500 p-2 bg-gray-50 border border-dashed border-gray-200 rounded-sm",
        }}
        {...props}
      />
      {helperText && <p className={`${color === "failure" ? "text-red-500" : "text-gray-500"} mt-1`}>{helperText}</p>}
    </div>
  );
}
