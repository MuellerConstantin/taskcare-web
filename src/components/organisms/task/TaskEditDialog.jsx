...
export default function TaskEditDialog({taskId, show, onEdit, onClose}) {
  ...
  } = useSWR(data ? `/boards/${data.boardId}/statuses/${data.statusId}` : null,
  ...
  } = useSWR(data ? data.componentIds.map(componentId => `/boards/${data.boardId}/components/${componentId}`) : null,
    async (urls) => {
      return await Promise.all(urls.map(url => api.get(url).then(res => res.data)));
    }
  , [data]);
  ...
  } = useSWR(data ? `/boards/${data.boardId}/members/${data.assigneeId}` : null,
    (url) => api.get(url).then((res) => res.data), [data]);
  ...
  const editTask = useCallback(async (values, {setFieldError}) => {
    setEditLoading(true);
    setEditError(null);
    ...
    <Modal size="lg" show={show} onClose={onClose}>
      ...
      <Formik
        initialValues={{
          ...
          statusId: statusData ? { label: statusData.name, value: statusData.id } : "",
          assigneeId: userData ? { label: userData.username, value: assigneeData.id } : "",
          componentIds: componentsData ? componentsData.map((component) => ({ label: component.name, value: component.id })) : [],
          priority: data?.priority && priorityOptions.find((option) => option.value === data.priority) || "",
          dueDate: data?.dueDate ? new Date(data.dueDate) : undefined,
          dueTime: data?.dueDate ? new Date(data?.dueDate).toTimeString().slice(0, 5) : ""
        }}
        validationSchema={schema}
        onSubmit={(values, { setFieldError }) => editTask(values, { setFieldError })}
      >
        {(props) => (
          <form
