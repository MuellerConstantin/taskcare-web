...
export default function BoardAddDialog({show, onAdd, onClose}) {
  const api = useApi();

  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(false);

  const addBoard = useCallback(async ({ name, description }, {setFieldError}) => {
    setLoading(true);
    setError(null);
  ...
  return (
    <Modal size="md" show={show} onClose={onClose}>
      ...
      <Formik
        ...
        onSubmit={(values, { setFieldError }) => addBoard(values, { setFieldError })}
      >
        {(props) => (
          <form
