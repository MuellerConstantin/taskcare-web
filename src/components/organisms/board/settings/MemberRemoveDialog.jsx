...
export default function MemberRemoveDialog({
  ...
  const removeMembers = useCallback(async () => {
    ...
    Promise.all(
      memberIds.map((id) => api.delete(`/boards/${boardId}/members/${id}`)),
    )
      .then(onRemove)
      .catch((err) => {
        if (
    ...
    <Modal size="md" show={show} onClose={onClose}>
      ...
      <Modal.Body className="flex flex-col space-y-4 text-gray-900 dark:text-white">
        ...
        <p className="text-xs">
          ...
          {memberIds.length > 1 ? "These members" : "This member"} will be
          permanently removed from the board. Please be certain before
          proceeding.
        </p>
      </Modal.Body>
