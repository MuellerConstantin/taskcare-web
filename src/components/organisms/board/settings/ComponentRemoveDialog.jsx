...
export default function ComponentRemoveDialog({show, boardId, onRemove, onClose, componentIds}) {
  const api = useApi();
  ...
  const removeComponentes = useCallback(async () => {
    ...
    Promise.all(componentIds.map((id) => api.delete(`/boards/${boardId}/components/${id}`)))
    .then(onRemove)
    .catch((err) => {
  ...
  return (
    <Modal size="md" show={show} onClose={onClose}>
      ...
      <Modal.Body className="space-y-4 flex flex-col text-gray-900 dark:text-white">
        ...
        <p className="text-xs">
          This action cannot be undone. {componentIds.length > 1 ? "These components" : "This component"} will
          be permanently deleted all the related tasks will be moved to the default component. A possibly
