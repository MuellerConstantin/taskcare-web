...
export default function StatusRemoveDialog({
  show,
  boardId,
  onRemove,
  onClose,
  ...
  const removeStatuses = useCallback(async () => {
    ...
    Promise.all(
      statusIds.map((id) => api.delete(`/boards/${boardId}/statuses/${id}`)),
    )
      .then(onRemove)
      .catch((err) => {
    ...
    <Modal size="md" show={show} onClose={onClose}>
      ...
      <Modal.Body className="flex flex-col space-y-4 text-gray-900 dark:text-white">
        ...
        <p className="text-xs">
          ...
          {statusIds.length > 1 ? "These statuses" : "This status"} will be
          permanently deleted all the related tasks will be moved to the default
          status. A possibly linked board layout column will also be deleted.
        </p>
      </Modal.Body>
