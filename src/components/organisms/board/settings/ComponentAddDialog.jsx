...
export default function ComponentAddDialog({show, boardId, onAdd, onClose}) {
  const api = useApi();

  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(false);

  const addComponent = useCallback(async ({ name, description }, {setFieldError}) => {
    setLoading(true);
    setError(null);

    api.post(`/boards/${boardId}/components`, {
      name,
  ...
  return (
    <Modal size="md" show={show} onClose={onClose}>
      ...
      <Formik
        ...
        onSubmit={(values, { setFieldError }) => addComponent(values, { setFieldError })}
      >
        {(props) => (
          <form
          ...
          >
            ...
            <Modal.Footer className="justify-end">
              ...
              >
                {!loading && <span>Add Component</span>}
