...
export default function LeaveBoardModal({
  boardId,
  onSubmit,
  onClose,
  isOpen,
  ...
  const onSubmitModal = async () => {
    ...
    try {
      await deleteMember(boardId, currentMember.username);
      onSubmit();
    } catch (err) {
      if (err.response && err.response.status === 401) {
        ...
        setError(
          "After leaving the board, it would no longer be administrable."
        );
      } else {
        ...
        <div className="min-h-screen px-4 text-center">
          ...
          >
            <Dialog.Panel className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl space-y-6 bg-white dark:bg-gray-800 text-gray-800 dark:text-white">
              ...
              <div>
                Are you sure you want to leave this board? The following things
                take effect immediately:
              </div>
              <div>
                <ul className="list-disc pl-6">
                  <li>
                    You lose all read and write permissions for the board.
                  </li>
                  <li>You won&quot;t be able to see this board longer.</li>
                </ul>
              </div>
