...
export default function BoardSettingsComponents() {
  const api = useApi();
  ...
  const [showComponentRemoveDialog, setShowComponentRemoveDialog] =
    useState(false);
  ...
  const {
    ...
    boardId
      ? `/boards/${boardId}/components?page=${page - 1}&perPage=${perPage}${searchQuery ? `&search=${searchQuery}` : ""}`
      : null,
